...
- 🟡 **Auth + entitlement foundation built** (Supabase EU magic-link, `is-Pro?` checks, usage-metering plumbing, payment seam stubbed — branch `auth-supabase`) — but **no live payments yet** (checkout gated on the legal entity).
...
**Finance / Pricing** — *State:* Tiers + billing decided (Free / Pro €10–13 / Team €8–10 per seat / Enterprise; Lemon Squeezy). **Auth + entitlement foundation BUILT** (Supabase EU magic-link, `is-Pro?` reads, usage-metering schema, payment seam stubbed — branch `auth-supabase`, unmerged); live checkout still not built. *Blocker:* no entity (can't take payments); hiding the key means Landa pays per cloud-transcription minute, pressuring margin. *Next:* proxy-side enforcement (server-side metering + free-tier limit), then wire Lemon Squeezy checkout to the seam once the entity exists; validate the Pro margin; confirm the free-tier word-reset cadence.

**Ops** — *State:* Proxy source now backed up to private GitHub; releases ship via tagged CI (notarized macOS + Windows). **Supabase project `landa` (EU/Frankfurt) provisioned; Brevo (EU) SMTP wired for auth emails.** *Blocker:* none acute. *Next:* **authenticate `landavoice.com` in Brevo (SPF/DKIM/DMARC) so auth mail leaves spam — pre-launch**; confirm Upstash rate-limit region is EU; keep the rate-limit guard sized for load.
