...
- **Streaming transcription = new primary-path direction (2026-05-25, research/decision session).** Decided: cloud streaming ASR is back IN as the primary path (wedge is EU-hosted+zero-retention, not on-device); local `landa-base` becomes the offline fallback shown in the UI; **no** large local polish model (download/RAM cost). Triggered by a teardown of a real Wispr Flow client log (audio streamed to Baseten *during* recording → 45.9s clip transcribes in 441ms; fused ASR→format; context prefetched during recording). Logged to STRATEGY Decision Log (3 rows, 2026-05-25) + Status (Product/Engineering/Compliance) + roadmap (flipped the "streaming OUT" line). New plan doc: [tasks/streaming-asr-latency-plan.md](tasks/streaming-asr-latency-plan.md). **⚠️ Gated** on the entity + EU-vendor track (no EU streaming endpoint exists; Vercel can't host the stream). **Buildable now (unblocked):** offline-path quality (deterministic vocab correction, `tasks/local-vocab-correction.md`) + offline UI badge + client streaming groundwork behind a flag. **Docs-only this session — no code changed.** ⚠️ Surface ripple when shipped: onboarding/landing/changelog all describe on-device transcription today.
